...
export default {
    ...
    mounted() {
      this.tripType = this.$route.params.tripType        //路由传参 获取当前的路由
      if(!this.tripType){
        this.$router.push({ path:'/trip' })   //没有选择出行方式往上跳回页面
      } else {
        this.createTrip()
      }
    },
    methods:{
        ...
        createTrip(){
          ...
          this.map.plugin(['AMap.Geolocation','AMap.ControlBar'], function() {
            ...
            // 定位插件
          that.map.addControl(geolocation)           //添加一个控制器控制geolocation
